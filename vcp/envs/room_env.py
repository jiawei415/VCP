...
# Multiworld Imports
from vcp.envs.env_util import get_stat_in_paths, create_stats_ordered_dict, get_asset_full_path
from vcp.envs.multitask_env import MultitaskEnv
from vcp.envs.mujoco_env import MujocoEnv
from vcp.envs.serializable import Serializable
import mujoco_py
...
class RoomEnv(MujocoEnv, MultitaskEnv, Serializable):
    ...
    def compute_rewards(self, actions, obs):
        ...
        rewards = self.compute_reward(achieved_goals, desired_goals)
        # rewards = np.array([
    ...
    def step(self, action):
        ...
        reward = self.compute_reward(obs['state_achieved_goal'], obs['state_desired_goal'])
        info = self._get_info(obs)
        self.curr_path_length += 1
        done = False # self.curr_path_length >= self.max_path_length
